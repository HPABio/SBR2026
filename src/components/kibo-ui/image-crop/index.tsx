...
const getCroppedPngImage = async (
  ...
  // If circular crop is enabled, mask to a circle and keep transparent corners.
  if (circularCrop) {
...
export type ImageCropProps = {
  ...
  maxImageSize?: number;
  onCrop?: (croppedImage: string) => void;
  /** Only used to set the initial crop box. */
...
export const ImageCrop = ({
  ...
  const onImageLoad = useCallback(
    (e: SyntheticEvent<HTMLImageElement>) => {
      ...
      const newCrop = centerAspectCrop(width, height, initialAspect ?? reactCropProps.aspect);
      setCrop(newCrop);
      setInitialCrop(newCrop);
    },
    [initialAspect, reactCropProps.aspect]
  );
  ...
  const applyCrop = async () => {
    ...
    const croppedImage = await getCroppedPngImage(
      ...
      completedCrop,
      maxImageSize,
      Boolean(reactCropProps.circularCrop)
    );
